...
BEGIN
	SET NOCOUNT ON;
	DECLARE 
		@ProcName NVARCHAR(128) = OBJECT_NAME(@@PROCID);--N'dbo.ParameterUnitSet';--
		...
		BEGIN TRAN
			...
			BEGIN														
				IF EXISTS(SELECT 1 FROM dbo.Units WHERE UnitID != @UnitID and UnitShortName = @UnitShortName AND LoginID = @LoginID)
					RAISERROR('Уже есть ед. измерения с таким кратким названием!',16,9);																		
			END;	
			...
			ELSE
				...
				ELSE					 
					...
					BEGIN
						...
						BEGIN
							RAISERROR('Ед.измерения используется в параметрах!',16,10);
						END	
