		...
		BEGIN TRAN
			...
			BEGIN
				IF EXISTS(SELECT 1 FROM dbo.Packets WHERE LoginID = @LoginID AND PacketShortName = @PacketShortName)
					RAISERROR('”же есть пакет с таким названием!',16,3);				
			...
			BEGIN
				...
				WHEN NOT MATCHED BY SOURCE THEN
					DELETE -- TODO: провер€ть, используетс€ ли в мониторе
				WHEN MATCHED THEN
					UPDATE 
